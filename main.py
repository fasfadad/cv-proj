...
def recsystem():
    ...
    def compareresultsandlocation():
        ...
        results = []
        for i in hotelmain :
            if hotelmain[i]["location"] == location :
                if hotelmain[i]['hotelclass'] == hotelrating :
                    if hotelmain[i]["price"] <= price:
                        if hotelmain[i]["freebreakfast"] == breakfast or -1:
                            if hotelmain[i]["shuttlebusservice"] == shuttlebus or -1:
                              if hotelmain[i]["spa"] == spa or -1:
                                   results.append(hotelmain[i])
